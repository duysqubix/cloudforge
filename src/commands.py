...
class TerraformCommands(BaseCommand):
    def setup(self) -> None:
        ...
        self.tmp_dir = self.tokenizer.dump_to(tree=parsed_tree, dirpath=TMP_PATH, unique=True)
    ...
    def execute(self):
        if self.targeted_action in ('validate', 'deploy'):        
            with TerraformInstaller(keep_binary=True) as tf_installer:
                ...
                tf = Terraform(terraform_bin_path=tf_installer.bin_path, working_dir=ectf_dir, is_env_vars_included=True)
                
                
                ret_code, msg, err = tf.init(backend_config=self.backend_config, backend=True, capture_output=None, reconfigure=False)
                
                if self.targeted_action == "validate":
                    ret_code, msg, err = tf.plan(str(self.tmp_dir.absolute()), capture_output=None)
                    print(msg, err)
    ...
    def _handle_debug_action(self):
            if platform.system() != "Linux":
                raise OSError("Debug command does not work in a windows environment, please switch to an OS that supports Bash Scripting")
...
class ECArgParser(ArgumentParser):
    ...
    def init(self):
        ...
        self.sub_parser = self.add_subparsers(help='Available subcommands', dest='subcmd')
    ...
    def _setup_terraform_subcommand(self):
        tf_subcmd = self.sub_parser.add_parser('tf', help='Terraform available commands', prefix_chars='tf-')
        tf_subcmd.add_argument("action", choices=['validate', 'deploy', 'debug', 'clean'], help="Choose an action")
        tf_subcmd.add_argument("env", choices=['dev', 'stg', 'uat', 'prod'], help="Choose an targeted environment")
        tf_subcmd.add_argument("-d", "--proj-dir", default=str(Path.cwd().absolute()), type=lambda x: Path(x), help=f"Define the project directory that holds ECTF files. Default: {str(Path.cwd().absolute())}")

    def _setup_clean_subcommand(self):
        clean_subcmd = self.sub_parser.add_parser("clean", help="helper utilities to clean up artifacts left by ec", prefix_chars="c-")
        clean_subcmd.add_argument("module", choices=['ectf', 'all'], help="Clean up artifacts left by ec.")
